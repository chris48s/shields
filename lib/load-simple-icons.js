...
function loadSimpleIcons() {
  ...
  const originalSlugs = Object.values(originalSimpleIcons).map(obj => obj.slug)
  Object.values(originalSimpleIcons).forEach(icon => {
    ...
    if (!(title in originalSlugs)) {
      simpleIcons[title] = icon
    }
    if (!(legacyTitle in originalSlugs)) {
